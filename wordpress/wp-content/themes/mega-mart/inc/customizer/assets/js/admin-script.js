( function( $ ){
    $( document ).ready( function(){
      $( '.mega-mart-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( mega_mart_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'mega_mart_dismissed_notice_handler',
              type: type,
            }
          } );
      } );

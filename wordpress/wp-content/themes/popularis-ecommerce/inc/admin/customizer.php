 ...
 *
 */
$popularis_ecommerce_sections = array('info', 'demo');

foreach ($popularis_ecommerce_sections as $section) {
    require get_template_directory() . '/inc/admin/customizer/' . $section . '.php';
}

function popularis_ecommerce_customizer_scripts() {
...
add_action('customize_controls_enqueue_scripts', 'popularis_ecommerce_customizer_scripts');
...
$config_customizer = array(
    ...
    'recommended_plugins_title' => sprintf(esc_html__('Welcome to %1$s - Version %2$s', 'popularis-ecommerce'), esc_html($theme_data->Name), esc_html($theme_data->Version)),
    'install_button_label' => esc_html__('Install now', 'popularis-ecommerce'),
    'activate_button_label' => esc_html__('Activate', 'popularis-ecommerce'),
);

Popularis_Customizer_Notice::init(apply_filters('popularis_ecommerce_customizer_notice_array', $config_customizer));

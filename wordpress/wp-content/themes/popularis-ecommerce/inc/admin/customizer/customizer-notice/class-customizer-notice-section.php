...
class Popularis_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function json() {
		$json = parent::json();
		global $popularis_ecommerce_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'popularis_ecommerce_show_recommended_plugins' );

		foreach ( $popularis_ecommerce_recommended_plugins as $slug => $plugin_opt ) {
			...
			$popularis_ecommerce_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$popularis_ecommerce_recommended_plugin['class'] = 'active';
			} else {
				$popularis_ecommerce_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$popularis_ecommerce_recommended_plugin['button_class'] = 'install-now button';
					$popularis_ecommerce_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$popularis_ecommerce_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$popularis_ecommerce_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$popularis_ecommerce_recommended_plugin['button_class'] = 'deactivate-now button';
					$popularis_ecommerce_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$popularis_ecommerce_recommended_plugin['id'] = $slug;
			$popularis_ecommerce_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$popularis_ecommerce_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$popularis_ecommerce_recommended_plugin['description'] = $info->short_description;
			}

			$popularis_ecommerce_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $popularis_ecommerce_recommended_plugin;

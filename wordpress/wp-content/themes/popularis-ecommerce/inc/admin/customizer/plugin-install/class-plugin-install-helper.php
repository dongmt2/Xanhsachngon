	...
	class Popularis_Plugin_Install_Helper {
			...
			if ( ! empty( $slug ) ) {
				...
				switch ( $state ) {
					case 'install':
						...
						$button .= '<a data-slug="' . esc_attr( $slug ) . '" class="install-now ta-install-plugin button  " href="' . esc_url( $nonce ) . '" data-name="' . $slug . '" aria-label="' . esc_attr__( 'Activate', 'popularis-ecommerce' ) . '">' . esc_html__( 'Activate', 'popularis-ecommerce' ) . '</a>';
						break;
					...
					case 'activate':					
						...
						$button .= '<a data-slug="' . esc_attr( $slug ) . '" class="activate button button-primary" href="' . esc_url( $nonce ) . '" aria-label="' . esc_attr__( 'Activate', 'popularis-ecommerce' ) . '">' . esc_html__( 'Activate', 'popularis-ecommerce' ) . '</a>';
						break;
				}// End switch().
				$button .= '</div>';
			}// End if().
		...
		public function enqueue_scripts() {
			...
			wp_localize_script(
				...
				array(
					'activating' => esc_html__( 'Activating', 'popularis-ecommerce' ),
				)
			);
			wp_localize_script(

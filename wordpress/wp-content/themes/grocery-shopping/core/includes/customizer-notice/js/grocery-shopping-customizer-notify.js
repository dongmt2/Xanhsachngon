...
(function (api) {
	...
	api.sectionConstructor['grocery-shopping-customizer-notify-section'] = api.Section.extend(
		{
				...
				return true;
					...
					jQuery( document ).ready(
						function () {
							...
							jQuery( '.grocery-shopping-customizer-notify-dismiss-recommended-action' ).click(
								function () {
									...
									jQuery.ajax(
										{
											type: 'GET',
											data: {action: 'grocery_shopping_customizer_notify_dismiss_action', id: id, todo: action},
											dataType: 'html',
											url: groceryshoppingCustomizercompanionObject.ajaxurl,
											beforeSend: function () {
												jQuery( '#' + id ).parent().append( '<div id="temp_load" style="text-align:center"><img src="' + groceryshoppingCustomizercompanionObject.base_path + '/images/spinner-2x.gif" /></div>' );
											},
											success: function (data) {
												...
												var actions_count      = recommended_sction.find( '.grocery-shopping-customizer-plugin-notify-actions-count' );
												var section_title      = recommended_sction.find( '.section-title' );
												jQuery( '.grocery-shopping-customizer-plugin-notify-actions-count .current-index' ).text( index );
												container.slideToggle().remove();
												if (jQuery( '.grocery-shopping-theme-recomended-actions_container > .epsilon-recommended-actions' ).length === 0) {

													actions_count.remove();

													if (jQuery( '.grocery-shopping-theme-recomended-actions_container > .epsilon-recommended-plugins' ).length === 0) {
										...
										jQuery( '.grocery-shopping-customizer-notify-dismiss-button-recommended-plugin' ).click(
											function () {
												var id = jQuery( this ).attr( 'id' ),
												action = jQuery( this ).attr( 'data-action' );
												jQuery.ajax(
										...
										function activatePlugin(url) {
										    ...
										    jQuery.ajax({
										        beforeSend: function() {
										            jQuery('.activate-now').replaceWith('<a class="button updating-message">' + groceryshoppingCustomizercompanionObject.activating_string + '...</a>');
										        },
										        async: true,
										        type: 'GET',
										        url: url,
...
jQuery( document ).ready(
	function ($) {
		...
		$( 'body' ).on(
			'click', ' .grocery-shopping-install-plugin ', function () {
